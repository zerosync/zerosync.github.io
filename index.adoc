...
=== Android/iOS Client
Phone and Tablets usually only a limited storage. Hence syncing everything will probably fail at some point. User nowadays view pictures, listen music and watch video on these devices. Therefore the Mobile App should only sync a subset of these files. Automatically switch videos/movies when they've been watched and load other from a prioritized list could be one feature for the app.  An option the app must provide is to turn off syncing when only mobile Internet is available.
...
* Web App http://web.zerosync.org[zeroserver] | http://wiki.webzerosync.org[wiki]
